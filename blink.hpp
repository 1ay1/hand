...
#ifndef HAND_BLINK_HPP
#define HAND_BLINK_HPP
...
struct SquareWave {
    static constexpr std::uint64_t period_ms = PeriodMs;

    // The wave's state at time `t`: high for [0, period/2), low for the rest.
    [[nodiscard]] static constexpr bool on(Millis t) noexcept {
        return (t.value / period_ms) % 2 == 0;
    }
};

// The cursor blinks on a ~530ms half-cycle; SGR-5 text on a slower ~750ms one.
struct CursorTag {};
struct TextTag {};
